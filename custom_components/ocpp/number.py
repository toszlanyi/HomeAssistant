...
class ChargePointNumber(RestoreNumber, NumberEntity):
    ...
    async def async_set_native_value(self, value):
        """Set new value for max current with 0/6-16A logic."""

        # Logik: Wenn Wert zwischen 0 und 6 liegt, entscheide zwischen 0 und 6
        target_value = float(value)
        if 0 < target_value < 6:
            # Wenn man näher an der 0 ist, setze 0, sonst 6
            target_value = 0.0 if target_value < 3 else 6.0
            _LOGGER.info("Ungültiger Bereich (1-5A) gewählt. Korrigiere auf %.1f A", target_value)

        # Update den Slider in der UI (er springt dann visuell auf 0 oder 6)
