...
class ChargePoint(cp):
    ...
    async def set_charge_rate(self, limit_amps=None, limit_watts=None, conn_id=0, profile=None):
        """Setzt die Laderate mit Compleo-kompatibler Logik (0 oder 6-16A)."""
        if profile is None:
            # 1. Wert ermitteln
            limit = limit_amps if limit_amps is not None else limit_watts / 230

            # 2. Original-Logik anwenden: Alles zwischen 0 und 6 auf 6 aufrunden, außer es ist 0
            if 0 < limit < 6:
                _LOGGER.info("Angeforderter Strom %.1f A unter Minimum. Korrigiere auf 6 A.", limit)
                limit = 6.0

            unit = ChargingRateUnitType.amps if limit_amps is not None else ChargingRateUnitType.watts

            # 3. Profil für eBox aufbauen
            profile = {
                "chargingProfileId": int(time.time() % 1000),
                "stackLevel": 1,
                "chargingProfilePurpose": ChargingProfilePurposeType.tx_default_profile.value,
                ...
                "chargingSchedule": {
                    ...
                    "startSchedule": datetime.now(tz=UTC).isoformat()
                },
            }

        _LOGGER.debug("Sende SetChargingProfile an eBox: %s", profile)
        ...
        try:
            resp = await self.call(req)
            if resp.status == ChargingProfileStatus.accepted.value:
                _LOGGER.info("eBox hat Limit von %.1f A übernommen.", limit)
                return True

            _LOGGER.warning("eBox hat das Profil abgelehnt: %s", resp.status)
        except Exception as e:
            _LOGGER.error("Fehler beim Senden des Ladeprofils: %s", e)
